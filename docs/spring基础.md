...
- 在`singleton` 加 `setter` 模式下，循环依赖是没有问题的，Spring能够自动解决（先曝光，后赋值）。
...
要想解刨一个类，必须先要获取到该类的 `Class 对象`，而刨析一个类或使用反射解决具体问题就是使用相关API的过程。**Class 对象是反射的根源**。
